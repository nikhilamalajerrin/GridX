...
- API behavior changes without checking `gridx/postman` and `gridx/gridx.io`.
...
Some changes require documentation updates in `gridx/gridx.io`.

Some API-related changes additionally require updates to API specification files in `gridx/postman`.
...
If API behavior changes, the agent must determine whether corresponding specification updates are needed in `gridx/postman`.
...
If API specification files are updated in `gridx/postman`, the agent should also determine whether a documentation PR or regeneration step is required in `gridx/gridx.io`.
...
3. reference the expected documentation area in `gridx/gridx.io`;
4. reference any required specification updates in `gridx/postman`.
