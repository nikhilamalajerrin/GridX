...
namespace GridX\RegistryBridge\Http\Controllers\Internal\v1;
...
use GridX\RegistryBridge\Models\RegistryExtensionPurchase;
use GridX\RegistryBridge\Support\Utils;
...
class RegistryPaymentsController extends Controller
{
     ...
     *
     * This method utilizes the GridX utility class to create a Stripe Express account and saves the
    ...
    public function getStripeCheckoutSessionStatus(Request $request)
    {
        ...
        if (session('company')) {
            ...
            $purchaserType = 'GridX\\Models\\Company';
        }
        // Check if authenticated via bearer token (developer account)
        elseif ($request->bearerToken()) {
            $registryUser = \GridX\RegistryBridge\Models\RegistryUser::where('token', $request->bearerToken())->first();
            if ($registryUser && $registryUser->developer_account_uuid) {
                $purchaserUuid = $registryUser->developer_account_uuid;
                $purchaserType = 'GridX\\RegistryBridge\\Models\\RegistryDeveloperAccount';
            }
        }
            ...
            if (isset($session->status) && $session->status === 'complete') {
                RegistryExtensionPurchase::firstOrCreate(
                    ...
                    [
                        ...
                        'company_uuid'               => $purchaserType === 'GridX\\Models\\Company' ? $purchaserUuid : null,
                    ]
                );
            }
    ...
    public function getAuthorReceivedPayments(Request $request)
    {
        ...
        return GridXResource::collection($payments)->additional(['total_amount' => $totalPurchaseAmount]);
    }
