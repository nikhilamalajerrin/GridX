...
namespace GridX\RegistryBridge\Models;
...
use GridX\RegistryBridge\Exceptions\InstallFailedException;
...
class RegistryExtensionBundle extends Model
{
    ...
    protected static function extractBundleFile(File $bundle, $filenames = 'extension.json', $options = []): ?\stdClass
    {
        ...
        $tempDir         = sys_get_temp_dir() . '/' . str_replace(['.', ','], '_', uniqid('gridx_archive_', true));
        mkdir($tempDir);
    ...
    public function installComposerPackage(): void
    {
        ...
        $process->setWorkingDirectory('/gridx/api');
    ...
    public function installEnginePackage(): void
    {
        ...
        $process->setWorkingDirectory(config('gridx.console.path'));
        $process->setTimeout(3600 * 2);
    ...
    /**
     * Initiates and manages the build process of the GridX console using PNPM.
     *
     * This method prepares and executes the build process for the GridX console. It monitors the build
