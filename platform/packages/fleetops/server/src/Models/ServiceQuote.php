...
use GridX\FleetOps\Integrations\Lalamove\Lalamove;
use GridX\FleetOps\Support\Payment;
use GridX\FleetOps\Support\Utils;
use GridX\Models\Model;
use GridX\Traits\Expirable;
...
class ServiceQuote extends Model
{
    ...
    public function getStripeProduct(): ?\Stripe\Product
    {
        ...
        if ($stripe) {
            ...
            if (!$product) {
                $product = $stripe->products->create(['name' => $this->public_id, 'description' => 'Service Quote for ' . Utils::moneyFormat($this->amount, $this->currency), 'metadata' => ['gridx_id' => $this->public_id]]);
            }
