...
use GridX\FleetOps\Events\OrderCompleted;
use GridX\FleetOps\Events\OrderDispatched;
use GridX\FleetOps\Events\OrderDriverAssigned;
use GridX\FleetOps\Flow\Activity;
use GridX\FleetOps\Support\LabelPdf;
use GridX\FleetOps\Support\OrderTracker;
use GridX\FleetOps\Support\Utils;
use GridX\FleetOps\Traits\HasTrackingNumber;
...
use GridX\Traits\HasCustomFields;
use GridX\Traits\HasInternalId;
use GridX\Traits\HasMetaAttributes;
use GridX\Traits\HasOptionsAttributes;
...
class Order extends Model
{
    ...
    public static function boot(): void
    {
        ...
        Relation::morphMap(
            [
                'GridX\\Models\\Contact'   => Contact::class,
                '\\GridX\\Models\\Contact' => Contact::class,
                'GridX\\Models\\Driver'    => Driver::class,
                '\\GridX\\Models\\Driver'  => Driver::class,
                'GridX\\Models\\Vendor'    => Vendor::class,
                '\\GridX\\Models\\Vendor'  => Vendor::class,
            ]
        );
    }
    ...
    public function attachFiles($uploads): self
    {
        ...
        \GridX\Models\File::whereIn('uuid', $ids)->get()->each(function ($file) {
            $file->setKey($this);
        });
    ...
    {
        return $this->hasMany(\GridX\Models\Comment::class, 'subject_uuid')->whereNull('parent_comment_uuid')->latest();
    }

    public function files(): HasMany
    {
        return $this->hasMany(\GridX\Models\File::class, 'subject_uuid')->latest();
    }
    ...
    public function cancel()
    {
        ...
        if ($this->isIntegratedVendorOrder()) {
            ...
            if (method_exists($api, 'cancelFromGridXOrder')) {
                $api->cancelFromGridXOrder($this);
            }
        }
