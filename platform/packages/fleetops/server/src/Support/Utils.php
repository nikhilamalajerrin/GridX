...
class Utils extends GridXUtils
{
     ...
     *
     * @param \GridX\FleetOps\Models\Place $place   the place object to retrieve the address from
     ...
     */
    public static function getAddressStringForPlace(\GridX\FleetOps\Models\Place $place, $useHtml = false, $except = [])
    {
    ...
    public static function getPointFromMixed($coordinates): ?Point
    {
        ...
        if ($coordinates instanceof \GridX\FleetOps\Models\Place) {
            $coordinates = $coordinates->location;
        }

        if ($coordinates instanceof \GridX\FleetOps\Models\Driver) {
            $coordinates = $coordinates->location;
        }

        if ($coordinates instanceof \GridX\FleetOps\Models\Vehicle) {
            $coordinates = $coordinates->location;
        }
        ...
        if ($coordinates instanceof \GridX\LaravelMysqlSpatial\Eloquent\SpatialExpression) {
        ...
        if (is_string($coordinates)) {
            ...
            if (Str::startsWith($coordinates, 'place_')) {
                $resolvedPlace = \GridX\FleetOps\Models\Place::where('public_id', $coordinates)->first();
                if ($resolvedPlace instanceof \GridX\FleetOps\Models\Place) {
            ...
            if (Str::startsWith($coordinates, 'driver_')) {
                $resolvedDriver = \GridX\FleetOps\Models\Driver::where('public_id', $coordinates)->first();
                if ($resolvedDriver instanceof \GridX\FleetOps\Models\Driver) {
            ...
            if (Str::isUuid($coordinates)) {
                $resolvedPlace = \GridX\FleetOps\Models\Place::where('uuid', $coordinates)->first();
                if ($resolvedPlace instanceof \GridX\FleetOps\Models\Place) {
                    return static::getPointFromMixed($resolvedPlace);
                }

                $resolvedDriver = \GridX\FleetOps\Models\Driver::where('uuid', $coordinates)->first();
                if ($resolvedDriver instanceof \GridX\FleetOps\Models\Driver) {
                    return static::getPointFromMixed($resolvedDriver);
                }
     ...
     *
     * @param \GridX\FleetOps\Models\Place|Point|array $origin
     * @param \GridX\FleetOps\Models\Place|Point|array $destination
     */
     ...
     *
     * @return \GridX\LaravelMysqlSpatial\Types\MultiPolygon|null the MultiPolygon object or null if not found
     */
    public static function createPolygonFromCountry(string $country): ?\GridX\LaravelMysqlSpatial\Types\MultiPolygon
    {
        ...
        if ($feature) {
            return \GridX\LaravelMysqlSpatial\Types\MultiPolygon::fromJson(json_encode($feature->geometry));
        }
     ...
     *
     * @param \GridX\LaravelMysqlSpatial\Types\Polygon $polygon the Polygon object from which to calculate the centroid
     *
     * @return array returns the centroid of the polygon as an array of coordinates
     */
    public static function getPolygonCentroid(\GridX\LaravelMysqlSpatial\Types\Polygon $polygon)
    {
     ...
     *
     * @param \GridX\LaravelMysqlSpatial\Types\MultiPolygon $multiPolygon the MultiPolygon object from which to calculate the centroid
     *
     * @return array returns the centroid of the multi-polygon as an array of coordinates
     */
    public static function getMultiPolygonCentroid(\GridX\LaravelMysqlSpatial\Types\MultiPolygon $multiPolygon)
    {
     ...
     *
     * @param \GridX\LaravelMysqlSpatial\Types\Polygon|null $polygon the Polygon object from which to extract coordinates
     *
     ...
     */
    public static function getCoordinatesFromPolygon(?\GridX\LaravelMysqlSpatial\Types\Polygon $polygon): array
    {
     ...
     */
    public static function getModelClassName($table, $namespaceSegments = '\\GridX\\FleetOps\\'): string
    {
     ...
     *
     * @return \GridX\LaravelMysqlSpatial\Eloquent\SpatialExpression|null returns a SpatialExpression object if the input is a valid GeoJSON, or null if the input is not valid
     *
     * @throws \InvalidArgumentException if the input is not a valid GeoJSON object
     */
    public static function createSpatialExpressionFromGeoJson($geoJson): ?\GridX\LaravelMysqlSpatial\Eloquent\SpatialExpression
    {
        ...
        $geo  = \GridX\LaravelMysqlSpatial\Types\Geometry::fromJson($json);

        // Return a new SpatialExpression object
        return new \GridX\LaravelMysqlSpatial\Eloquent\SpatialExpression($geo);
    }
     ...
     * @throws \InvalidArgumentException if the input is not a valid GeoJSON object
     */
    public static function createGeometryObjectFromGeoJson($geoJson): ?\GridX\LaravelMysqlSpatial\Types\Geometry
    {
        if (!static::isGeoJson($geoJson)) {
            return null;
        }
        ...
        return \GridX\LaravelMysqlSpatial\Types\Geometry::fromJson($json);
    }
