...
namespace GridX\Webhook;

use GridX\Webhook\BackoffStrategy\BackoffStrategy;
use GridX\Webhook\Exceptions\CouldNotCallWebhook;
use GridX\Webhook\Exceptions\InvalidBackoffStrategy;
use GridX\Webhook\Exceptions\InvalidSigner;
use GridX\Webhook\Exceptions\InvalidWebhookJob;
...
class WebhookCall
{
    ...
    public static function create(): self
    {
        ...
        return (new static())
            ...
            ->useBackoffStrategy(data_get($config, 'backoff_strategy', \GridX\Webhook\BackoffStrategy\ExponentialBackoffStrategy::class))
            ->timeoutInSeconds(data_get($config, 'timeout_in_seconds'))
            ->signUsing(data_get($config, 'signer', \GridX\Webhook\Signer\DefaultSigner::class))
