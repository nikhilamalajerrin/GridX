...
/**
 * Normalize geoiplookup.io response to match GridX whois format
 */
...
/**
 * Normalize ipapi.co response to match GridX whois format
 */
...
function getCachedWhois() {
    try {
        const cached = localStorage.getItem('gridx:whois');
        if (!cached) {
            return null;
        }
        ...
        if (age > maxAge) {
            localStorage.removeItem('gridx:whois');
            return null;
        }
...
function cacheWhois(data) {
    try {
        localStorage.setItem('gridx:whois', JSON.stringify(data));
    } catch (error) {
