...
class RegistryExtension extends Model
{
     ...
     * `uuid`, `public_id`, and `slug`. If the package name starts with 'gridx/', it also attempts to match the slug extracted from the package name.
     *
    ...
    public static function lookup(string $packageName): ?RegistryExtension
    {
        return static::where('status', 'published')->where(function ($query) use ($packageName) {
            ...
            // Check for gridx/ prefix and match slug
            if (Str::startsWith($packageName, 'gridx/')) {
    ...
    public function getStripeProduct(): ?\Stripe\Product
    {
        ...
        if ($stripe) {
            ...
            if (!$product) {
                $product = $stripe->products->create(['name' => $this->name, 'description' => $this->description, 'metadata' => ['gridx_id' => $this->public_id]]);
            }
    ...
    public function createStripeCheckoutSession(string $returnUri): \Stripe\Checkout\Session
    {
        ...
        // Calculate the fee gridx takes for faciliation of extension
