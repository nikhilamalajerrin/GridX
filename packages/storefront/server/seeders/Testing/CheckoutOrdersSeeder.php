...
use GridX\Storefront\Seeders\Testing\Concerns\SeedsTestingData;
...
class CheckoutOrdersSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $store = $this->seededModel(Store::class, 'store:gridx-market');
        if (!$store) {
    ...
    public function purgeSeedData(): void
    {
        ...
        $this->deleteFrom($this->gridxConnection(), 'transaction_items', fn ($query) => $query->whereIn('transaction_uuid', $transactionUuids)->orWhere('meta->seed', static::SEED_NAME));
    ...
    {
        if (!Schema::connection($this->gridxConnection())->hasTable('ledger_journals')) {
            return;
        }

        DB::connection($this->gridxConnection())
