...
namespace GridX\Storefront\Seeders\Testing\Concerns;

use GridX\Models\Company;
use GridX\Models\CompanyUser;
use GridX\Models\Model as GridXModel;
...
trait SeedsTestingData
{
    ...
    protected function prepareCompany(): ?Company
    {
        ...
        if (!$company) {
            $this->command?->error('No company found. Create a GridX company before running Storefront testing seeders.');
    ...
    protected function gridxConnection(): string
    {
        return config('gridx.connection.db');
    }
}
