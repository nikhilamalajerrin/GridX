...
use GridX\Services\AwsSnsSmsService;
use GridX\Services\CustomHttpSmsService;
use GridX\Services\MessageBirdSmsService;
use GridX\Services\SmppGatewayClient;
use GridX\Services\SmppSmsService;
use GridX\Services\SmsService;
use GridX\Services\VonageSmsService;
...
test('custom http sms service renders configured post templates', function () {
    ...
    Http::assertSent(function ($request) {
        return $request->method() === 'POST'
            ...
            && $request->hasHeader('X-Tenant', 'gridx')
            && $request['recipient'] === '+15551234567'
            && $request['message'] === 'Hello'
            && $request['sender'] === 'GridX'
...
test('smpp sms service validates config and delegates to client', function () {
    ...
    expect($result)->toMatchArray([
        ...
        ->and($client->submitted['from'])->toBe('GRIDX')
