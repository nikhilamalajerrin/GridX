...
namespace GridX\Expansions;

use Closure;
use GridX\Build\Expansion;
use GridX\Http\Controllers\Internal\v1\AuthController;
use GridX\Http\Middleware\ThrottleRequests;
use GridX\Routing\RESTRegistrar;
...
class Route implements Expansion
{
     ...
     * @return \Closure
     */
    public function gridxRestRoutes()
    {
        /*
         * Registers a REST complicit collection of routes.
         *
    ...
    public function gridxRoutes()
    {
        return function (string $name, callable|array|null $registerFn = null, $options = [], $controller = null) {
            ...
            $register = function ($router) use ($name, $registerFn, $make, $controller, $options) {
                ...
                $router->gridxRestRoutes($name, $controller, $options);
            };
    ...
    public function gridxAuthRoutes(): \Closure
    {
        return function (?string $authControllerClass = null, ?callable $registerFn = null, ?callable $registerProtectedFn = null) {
            ...
            $this->group(['prefix' => 'auth'], function (Router $router) use ($authControllerClass, $registerFn, $registerProtectedFn) {
                ...
                $router->group(['middleware' => ['gridx.protected']], function (Router $router) use ($authControllerClass, $registerProtectedFn) {
    ...
    public function registerGridXOnboardRoutes()
    {
        return function () {
            return $this;
        };
