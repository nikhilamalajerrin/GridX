...
class MigrateSandbox extends Command
{
    ...
    public function handle()
    {
        ...
        $paths                = ['vendor/gridx/core-api/migrations'];
        ...
        foreach ($paths as $path) {
            $this->call($command, [
                '--seed'     => $seed,
                '--force'    => $force,
                '--database' => config('gridx.connection.sandbox'),
                '--path'     => $path,
            ]);
        }
    }
    ...
    /**
     * Returns the relative paths to the migration directories of all installed GridX extensions.
     *
     * This function retrieves all installed GridX extensions, and then for each extension,
     ...
     *
     * @return array an array of the relative paths to the migration directories of all installed GridX extensions
     */
    private function getExtensionsMigrationPaths(): array
    {
        $packages = Utils::getInstalledGridXExtensions();
        $paths    = [];

        foreach ($packages as $packageName => $package) {
            // check if migrations is disabled for sandbox
            $sandboxMigrations = Utils::getGridXExtensionProperty($packageName, 'sandbox-migrations');
