...
class Utils
{
    ...
    public static function consoleUrl(string $path = '', ?array $queryParams = [], $subdomain = null): string
    {
        ...
        $isLocalDevelopment = app()->environment(['local', 'development']);
        $host               = config('gridx.console.host');
        $subdomain          = config('gridx.console.subdomain', $subdomain);
        ...
        if (!Str::startsWith($host, 'http')) {
            $segments[] = config('gridx.console.secure', !$isLocalDevelopment) ? 'https://' : 'http://';
        }

        // check for subdomain
        if (config('gridx.console.subdomain', $subdomain)) {
            $segments[] = $subdomain . '.';
        }
    ...
    {
        $basePath = rtrim(config('gridx.console.path', ''), DIRECTORY_SEPARATOR);
    ...
    /**
     * Return asset URL from GridX S3 asset bucket.
     *
     * @return bool
     */
    public static function assetFromGridX(string $path): string
    {
     ...
     */
    public static function getModelClassName($table, $namespaceSegments = '\\GridX\\'): string
    {
        if (is_object($table)) {
     ...
     *
     * storefront:store -> GridX\Storefront\Models\Store
     * fleet-ops:order -> GridX\FleetOps\Models\Order
     * user -> GridX\Models\User
     * GridX\Models\Order -> GridX\Models\Order
     *
    ...
    public static function getMutationType($type): string
    {
        ...
        if (Str::contains($type, ':')) {
            ...
            $package   = $namespace[0];
            $type      = $namespace[1];
            $namespace = 'GridX\\' . Str::studly($package);
    ...
    /**
     * Validate string if is valid gridx public_id.
     *
     * @param string $string
     *
     * @return bool
     ...
     *
     * Examples:
     * - \GridX\FleetOps\Models\IntegratedVendor -> fleet-ops:integrated-vendor
     * - \GridX\Fliit\Models\Client -> fliit:client
    ...
    public static function toEmberResourceType($className)
    {
        ...
        // e.g., "GridX\\Fliit\\Models\\Client" -> "fliit:client"
        // e.g., "GridX\\FleetOps\\Models\\Vendor" -> "fleet-ops:vendor"
        ...
        // Get the package name (second part of namespace after GridX)
        if (count($parts) >= 3 && $parts[0] === 'GridX') {
     ...
     * with the "gridx" key.
     *
     * @param string $key the key to search for in the "extra" property of packages with the "gridx" key
     *
     * @return array an array of values for the specified key from the "extra" property of packages with the "gridx" key
     *
     * @throws \RuntimeException If the installed.json file cannot be found.
     */
    public static function fromGridXExtensions(string $key): array
    {
        ...
        if (isset($installedPackages['packages'])) {
            foreach ($installedPackages['packages'] as $package) {
                if (isset($package['extra']['gridx']) && isset($package['extra']['gridx'][$key])) {
                    $gridxExtensions[] = $package['extra']['gridx'][$key];
                }
            }
        }

        return array_values($gridxExtensions);
    }
     ...
     * with the "gridx" key.
     *
     * @param string $extension the gridx extension to lookup the property for
     * @param string $key       the key to search for in the "extra" property of packages with the "gridx" key
     *
     ...
     * @throws \RuntimeException If the installed.json file cannot be found.
     */
    public static function getGridXExtensionProperty(string $extension, string $key)
    {
        $installedJsonPath = realpath(base_path('vendor/composer/installed.json'));
    ...
    /**
     * Retrieves the database name for the GridX connection from the configuration.
     *
     * @return string|null the database name for the GridX connection, or null if not found
     */
    public static function getGridXDatabaseName(): ?string
    {
        $connection   = config('gridx.connection.db');
    ...
    /**
     * Get installed GridX extensions.
     *
     * @return array
     */
    ...
    /**
     * Retrieves directories containing seeders from installed GridX extensions.
     *
     * This function first gets all installed GridX extensions. Then it iterates over them
     ...
     *
     * @return array the array containing the paths to seeder directories of all installed GridX extensions
     *
    ...
    {
        $packages      = static::getInstalledGridXExtensions();
        $seederClasses = [];
    ...
    /**
     * Get directories containing seed files from GridX extensions installed in the project.
     *
     * This method iterates over all packages installed in the project and identified as GridX extensions.
    ...
    /**
     * Retrieves directories containing migrations from installed GridX extensions.
     *
     * This function first gets all installed GridX extensions. Then it iterates over them
     * and checks for a 'migrations' directory within each one. If it exists, the directory path
     * is added to an array. The function finally returns this array of migration directories.
     *
     * @return array the array containing the paths to migration directories of all installed GridX extensions
     *
     * @throws \RuntimeException if an error occurs during directory retrieval
     */
    ...
    /**
     * Retrieves the migration directory for a specific GridX extension.
     *
     * This function first gets all installed GridX extensions. Then it iterates over them
     ...
     *
     * @param string $extension the name of the GridX extension for which the migration directory is to be retrieved
     *
     * @return string|null the path to the migration directory of the specified GridX extension, or null if the extension is not found
     *
     * @throws \RuntimeException if an error occurs during directory retrieval
     */
    ...
    /**
     * Get the namespaced names of the authentication schemas found in the installed GridX extensions.
     *
     * @return array
     */
    public static function getAuthSchemaNamespaces()
    {
        ...
        $localNamespace            = 'GridX\\';
        $localPackageSrcDirectory  = base_path('vendor/gridx/core-api/src/');
    ...
    /**
     * Get the authentication schemas instances from the installed GridX extensions.
     *
     * @return array
     */
     ...
     */
    public static function getDefaultMailFromAddress(?string $default = 'hello@gridx.io'): string
    {
