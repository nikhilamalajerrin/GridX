...
class GridXBlog
{
    ...
    public static function normalizeLink(?string $link, ?string $blogUrl = null): string
    {
        ...
        if ($host && (Str::contains($host, 'ghost.io') || $host === 'blog.gridx.io') && $path) {
            return $blogUrl . '/' . $path;
        }
    ...
    {
        return rtrim($feedUrl ?: getenv('GRIDX_BLOG_FEED_URL') ?: 'https://blog.gridx.io/rss/', '/') . '/';
    }
    ...
    {
        return rtrim($blogUrl ?: getenv('GRIDX_BLOG_URL') ?: 'https://www.gridx.io/blog', '/');
    }
