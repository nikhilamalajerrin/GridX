...
class Telemetry
{
     ...
     */
    protected static string $endpoint = 'https://telemetry.gridx.io/';
    ...
    public static function send(array $payload = []): bool
    {
        ...
        try {
            ...
            $tags   = [
                'gridx.instance_id:' . self::getInstanceId(),
                'gridx.company:' . self::getCompanyName(),
                'gridx.version:' . self::getVersion(),
                'gridx.domain:' . request()->getHost(),
                'gridx.api:' . config('app.url'),
                'gridx.console:' . config('gridx.console.host'),
                'gridx.app_name:' . config('app.name'),
                'gridx.version:' . config('gridx.version'),
            ...
            $defaultPayload = [
                'title'      => 'GridX Instance Telemetry',
                'text'       => 'Periodic instance telemetry from GridX',
                'tags'       => $tags,
                'alert_type' => 'info',
            ];
    ...
    {
        return config('gridx.instance_id', self::generateInstanceId());
    }
    ...
    /**
     * Get the current GridX version.
     */
    protected static function getVersion(): string
    {
        return config('gridx.version', '0.7.1');
    }
    ...
    {
        try {
            $response = Http::acceptJson()->get('https://api.github.com/repos/gridx/gridx/commits/main');
    ...
    protected static function getIpInfo(): array
    {
        ...
        try {
            self::$ipInfo = \GridX\Support\Http::lookupIp();
        } catch (\Throwable $e) {
