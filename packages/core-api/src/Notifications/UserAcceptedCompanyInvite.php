...
class UserAcceptedCompanyInvite extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage())
            ->subject($this->user->name . ' has joined ' . $this->company->name . ' on GridX!')
            ->greeting('Hello, Team!')
            ->line($this->user->name . ' has accepted the invitation and has joined ' . $this->company->name . ' on GridX.')
            ->line('Please welcome them to the team.')
            ->action('View Team Members', Utils::consoleUrl('iam/users'))
            ->line('Thank you for using GridX!');
    }
