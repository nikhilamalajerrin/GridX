...
class LookupController extends Controller
{
    ...
    /**
     * Pull the GridX.io blog RSS feed with aggressive caching.
     *
     * @return \Illuminate\Http\Response
     */
    public function gridxBlog(Request $request)
    {
        $limit    = max(1, min($request->integer('limit', 6), 20));
        $cacheKey = $this->getGridXBlogCacheKey($limit);
    ...
    protected function fetchBlogPosts(int $limit): array
    {
        ...
        $rssUrl = $this->getGridXBlogFeedUrl();
        $posts  = [];
    ...
    {
        return GridXBlog::parseRss($rssXml, $limit, $this->getGridXBlogUrl());
    }

    /**
     * Get the cache key for the GridX blog feed.
     */
    protected function getGridXBlogCacheKey(int $limit): string
    {
        $sourceHash = md5($this->getGridXBlogFeedUrl() . '|' . $this->getGridXBlogUrl());

        return "gridx_blog_posts_{$limit}_{$sourceHash}";
    }

    /**
     * Get the public GridX blog RSS feed URL.
     */
    protected function getGridXBlogFeedUrl(): string
    {
        return GridXBlog::getFeedUrl();
    }

    /**
     * Get the canonical GridX blog URL.
     */
    protected function getGridXBlogUrl(): string
    {
        return GridXBlog::getBlogUrl();
    }

    /**
     * Rewrite Ghost publication links to the canonical GridX.io blog URL.
     */
    protected function normalizeGridXBlogLink(?string $link): string
    {
        return GridXBlog::normalizeLink($link, $this->getGridXBlogUrl());
    }
    ...
    public function refreshBlogCache()
    {
        ...
        Cache::forget($this->getGridXBlogCacheKey(6));
        Cache::forget($this->getGridXBlogCacheKey(10));
        Cache::forget($this->getGridXBlogCacheKey(20));

        // Warm up cache with default limit
        $posts = $this->fetchBlogPosts(6);
        Cache::put($this->getGridXBlogCacheKey(6), $posts, now()->addDays(4));
