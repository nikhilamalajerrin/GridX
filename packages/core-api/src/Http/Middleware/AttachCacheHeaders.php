...
use GridX\Support\ApiModelCache;
...
class AttachCacheHeaders
{
     ...
     *
     * Uses GridX's Http helper to determine if the request is either:
     * - Internal request (int/v1/... - used by GridX applications)
     * - Public request (v1/... - used by end users for integrations/dev)
     */
    protected function isApiRequest(Request $request): bool
    {
        // Check if it's an internal request (int/v1/...)
        if (\GridX\Support\Http::isInternalRequest($request)) {
            return true;
        }

        // Check if it's a public API request (v1/...)
        if (\GridX\Support\Http::isPublicRequest($request)) {
            return true;
        }
