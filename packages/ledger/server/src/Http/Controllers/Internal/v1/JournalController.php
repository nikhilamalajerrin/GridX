...
use GridX\Ledger\Http\Resources\v1\Journal as JournalResource;
...
class JournalController extends LedgerResourceController
{
    ...
    public function createManual(Request $request): JsonResponse
    {
        ...
        $debitAccount = \GridX\Ledger\Models\Account::where('company_uuid', session('company'))
            ->where('uuid', $request->input('debit_account_uuid'))
            ->firstOrFail();

        $creditAccount = \GridX\Ledger\Models\Account::where('company_uuid', session('company'))
