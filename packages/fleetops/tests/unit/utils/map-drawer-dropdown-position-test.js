...
module('Unit | Utility | map-drawer-dropdown-position', function () {
    test('positions the dropdown to the left of the trigger', function (assert) {
        ...
        assert.strictEqual(result.style.position, 'fixed', 'dropdown content is positioned in the viewport');
        assert.strictEqual(result.style.left, 274, 'left edge is trigger left minus menu width and gap');
        assert.strictEqual(result.style.top, 300, 'top aligns to trigger top');
        assert.strictEqual(result.style.zIndex, 10000, 'z-index keeps the menu above the drawer');
    });
    ...
    test('clamps vertically without flipping above the trigger', function (assert) {
        ...
        assert.strictEqual(result.style.top, 434, 'top is clamped inside the drawer instead of flipped above the trigger');
    });
});
