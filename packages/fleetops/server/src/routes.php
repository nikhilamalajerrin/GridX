...
|
*/
Route::prefix(config('fleetops.api.routing.prefix'))->namespace('GridX\FleetOps\Http\Controllers')->group(
    function ($router) {
        /*
        |--------------------------------------------------------------------------
        ...
        */
        $router->group(['prefix' => 'v1', 'middleware' => ['gridx.api', GridX\FleetOps\Http\Middleware\TransformLocationMiddleware::class], 'namespace' => 'Api\v1'], function ($router) {
            // drivers routes
            ...
            $router->group(['prefix' => 'customers', 'middleware' => []], function () use ($router) {
                ...
                $router->group(['middleware' => [GridX\FleetOps\Http\Middleware\AuthenticateCustomerToken::class]], function () use ($router) {
            ...
            function ($router) {
                ...
                $router->group(
                    [
                        ...
                        'middleware' => [
                            'gridx.protected',
                            GridX\FleetOps\Http\Middleware\TransformLocationMiddleware::class,
                            GridX\FleetOps\Http\Middleware\SetupDriverSession::class,
                        ],
                    ],
                    function ($router) {
                        ...
                        $router->gridxRoutes('fuel-provider-sync-runs');
                        $router->gridxRoutes(
                        ...
                        $router->gridxRoutes('order-configs');
                        $router->gridxRoutes('payloads');
                        $router->gridxRoutes(
                            'places',
                            function ($router, $controller) {
                        ...
                        $router->gridxRoutes('proofs');
                        $router->gridxRoutes('purchase-rates');
                        $router->gridxRoutes('routes');
                        $router->gridxRoutes('positions', function ($router, $controller) {
                        ...
                        $router->gridxRoutes('tracking-numbers');
                        $router->gridxRoutes('tracking-statuses');
                        $router->gridxRoutes(
                            'vehicles',
                            function ($router, $controller) {
                                $router->get('statuses', $controller('statuses'));
                        ...
                        $router->gridxRoutes('vehicle-devices');
                        $router->gridxRoutes(
                            'vendors',
                            function ($router, $controller) {
                                $router->get('statuses', $controller('statuses'));
                        ...
                        $router->gridxRoutes('devices', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                        });
                        $router->post('devices/{id}/attach', 'DeviceController@attach');
                        $router->post('devices/{id}/detach', 'DeviceController@detach');
                        $router->gridxRoutes('device-events', function ($router, $controller) {
                            $router->post('{id}/mark-processed', $controller('markProcessed'));
                        });
                        $router->gridxRoutes('sensors', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                        });
                        $router->gridxRoutes('telematics', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->get('providers', $controller('providers'));
                        ...
                        $router->gridxRoutes('maintenance-schedules', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        ...
                        $router->gridxRoutes('work-orders', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                            $router->post('{id}/send', $controller('sendEmail'));
                        });
                        $router->gridxRoutes('maintenances', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        ...
                        $router->gridxRoutes('equipment', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        });
                        $router->gridxRoutes('parts', function ($router, $controller) {
                            $router->match(['get', 'post'], 'export', $controller('export'));
                            $router->post('import', $controller('import'));
                        });
                        $router->gridxRoutes('warranties');
                        $router->group(
                            ['prefix' => 'query'],
                            function () use ($router) {
