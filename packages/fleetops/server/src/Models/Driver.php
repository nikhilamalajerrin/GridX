...
use GridX\FleetOps\Scopes\DriverScope;
...
use GridX\LaravelMysqlSpatial\Types\Point as SpatialPoint;
use GridX\Models\File;
use GridX\Models\Model;
use GridX\Models\Schedule;
use GridX\Models\ScheduleAvailability;
...
class Driver extends Model
{
    ...
    {
        return $this->hasMany(\GridX\Models\UserDevice::class, 'user_uuid', 'user_uuid');
    }
    ...
    {
        // Explicit FK columns required — GridX uses *_uuid not *_id
    ...
    public static function getAvatarOptions(): Collection
    {
        ...
        // Create default avatars included from gridx
        $avatars = collect($options)->mapWithKeys(
            function ($option) {
                $key = str_replace(['.svg', '.png'], '', $option);

                return [$key => Utils::assetFromGridX('static/driver-icons/' . $option)];
            }
        );
