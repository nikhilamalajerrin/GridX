...
namespace GridX\FleetOps\Console\Commands;
...
class FixInvalidPolymorphicRelationTypeNamespaces extends Command
{
    ...
    {
        $models = [
            ['model' => \GridX\FleetOps\Models\Order::class, 'columns' => ['customer_type', 'facilitator_type']],
            ['model' => \GridX\FleetOps\Models\Place::class, 'columns' => ['owner_type']],
            ['model' => \GridX\FleetOps\Models\Entity::class, 'columns' => ['customer_type']],
            ['model' => \GridX\FleetOps\Models\PurchaseRate::class, 'columns' => ['customer_type']],
            ['model' => \GridX\FleetOps\Models\Device::class, 'columns' => ['attachable_type']],
        ];
        ...
        foreach ($records as $record) {
            foreach ($columns as $column) {
                if (isset($record->$column)) {
                    ...
                    if (Str::startsWith($normalizedOriginal, 'GridX\Models')) {
                        $newNamespace    = Str::replaceFirst('GridX\Models', 'GridX\FleetOps\Models', $normalizedOriginal);
