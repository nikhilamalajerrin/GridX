...
namespace GridX\FleetOps\Http\Controllers;

use GridX\FleetOps\Models\Telematic;
use GridX\FleetOps\Support\Telematics\TelematicProviderRegistry;
use GridX\FleetOps\Support\Telematics\TelematicService;
use GridX\Support\IdempotencyManager;
...
class TelematicWebhookController extends Controller
{
    ...
    public function handle(Request $request, string $providerKey): JsonResponse
    {
        ...
        // resolved by provider identifiers first; GridX URL/header ids are
        // optional configuration affordances where provider dashboards allow it.
        $telematicId = $request->query('telematic') ?? $request->query('integration') ?? $request->header('X-GridX-Telematic');
