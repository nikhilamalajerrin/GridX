...
export default class GithubCardComponent extends Component {
    ...
    @computed('data.releases_url', 'latestRelease.name') get releaseUrl() {
        let url = 'https://github.com/gridx/gridx/releases';
    ...
    @task *getRepositoryData() {
        ...
        const cachedData = this.localCache.get('gridx-github-data');
        const expiration = this.localCache.get('gridx-github-data-expiration');
        ...
        if (cachedData && expiration && !isPast(new Date(expiration))) {
            ...
            // Fetch new data
            const response = yield fetch('https://api.github.com/repos/gridx/gridx', { cache: 'default' });
            if (response.ok) {
                this.data = yield response.json();
                this.localCache.set('gridx-github-data', this.data);
                this.localCache.set('gridx-github-data-expiration', add(new Date(), { hours: 6 }));
            }
        }
    }
    ...
    @task *getRepositoryTags() {
        ...
        const cachedTags = this.localCache.get('gridx-github-tags');
        const expiration = this.localCache.get('gridx-github-tags-expiration');
        ...
        if (cachedTags && expiration && !isPast(new Date(expiration))) {
            ...
            // Fetch new tags
            const response = yield fetch('https://api.github.com/repos/gridx/gridx/tags', { cache: 'default' });
            if (response.ok) {
                this.tags = yield response.json();
                this.localCache.set('gridx-github-tags', this.tags);
                this.localCache.set('gridx-github-tags-expiration', add(new Date(), { hours: 6 }));
            }
        }
    }
}
