...
export default class GridXBlogComponent extends Component {
    ...
    @task *loadBlogPosts() {
        ...
        const cachedData = this.localCache.get('gridx-blog-data');
        const expiration = this.localCache.get('gridx-blog-data-expiration');
        ...
        if (cachedData && isArray(cachedData) && expiration && !isPast(new Date(expiration))) {
            ...
            try {
                ...
                if (data) {
                    ...
                    this.localCache.set('gridx-blog-data-expiration', add(new Date(), { hours: 6 }));
                }
            } catch (err) {
